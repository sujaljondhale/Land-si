...
function PrintHeader({ title, user }: { title: string, user: any }) {
  return (
    <div className="hidden print:block pb-6 mb-8 border-b-4 border-black break-after-avoid">
      <div className="flex items-center gap-6 mb-2">
        <div className="w-20 h-20 border-4 border-black rounded-full flex items-center justify-center font-extrabold text-4xl">
          🏛️
        </div>
        <div>
          <h1 className="text-4xl font-extrabold text-black uppercase tracking-widest mb-1">Government of India</h1>
          <h2 className="text-2xl font-bold text-gray-800">{title}</h2>
        </div>
      </div>
      <div className="flex justify-between text-sm font-bold text-gray-600 mt-6 pt-4 border-t border-gray-300">
        <p>Generated by: {user?.name || 'Authorized Personnel'} ({user?.role || 'Staff'})</p>
        <p>Date: {new Date().toLocaleDateString()} {new Date().toLocaleTimeString()}</p>
        <p>Document Ref: #SYS-{Math.floor(Math.random() * 100000)}</p>
      </div>
    </div>
  );
}
...
function AdminDashboard({ data, user }: { data: any, user: any }) {
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      <PrintHeader title="National Land Analytics Overview" user={user} />
...
function ResearchDashboard() {
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      <PrintHeader title="Academic Portal: Data Export" user={user} />
      <div className="flex justify-between items-start print:hidden">
        <div>
...
function PublicDashboard() {
  ...
  return (
    <div className="space-y-6 print:space-y-4">
      <PrintHeader title="Citizen Portal: Grievance & Records Export" user={user} />
      <div className="flex justify-between items-start print:hidden">
        <div>
